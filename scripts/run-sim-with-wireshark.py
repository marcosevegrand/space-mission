...
def main():
    ...
    # -------------------------------------------------------------
    # NEW: OPEN WIRESHARK ON ROVER 1
    # -------------------------------------------------------------
    log("🦈 Opening Wireshark inside 'rover1' node...")

    # We pass the host DISPLAY so the GUI appears on your screen
    display_val = os.environ.get("DISPLAY", ":0")

    # -k starts capturing immediately, -i eth0 selects the default interface
    cmd_wireshark = f"DISPLAY={display_val} wireshark -k -i eth0"

    p_ws = run_vcmd(
        session_dir, NODES["rover1"]["name"], cmd_wireshark, "wireshark_rover1"
    )
    if p_ws:
        procs.append(p_ws)
    # -------------------------------------------------------------

    # Launch actual Rover Binaries
